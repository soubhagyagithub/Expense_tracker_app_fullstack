...
function addRowToTable(tableBody, expenseDetails) {
    ...
    categoryCell.textContent = expenseDetails.catagory;
    ...
    deleteButton.onclick = () => {
        console.log('Deleting user with ID:', expenseDetails.id);
        axios.delete(`http://localhost:3000/user/delete-expense/${expenseDetails.id}`)
        .then((res) =>{
    ...
    axios.get('http://localhost:3000/user/get-expense')
        .then((res) => {
            for (let i = 0; i < res.data.allUsers.length; i++) {
                const expenseDetails = res.data.allUsers[i];
...
function handleFormSubmit(event) {
    ...
    for (let i = 0; i < tableBody.rows.length; i++) {
        ...
        if (row.classList.contains('editing')) {
            ...
            axios.put(`http://localhost:3000/user/update-expense/${expenseDetails.id}`,expenseDetails)
                .then((res) => {
    ...
    //    making post request for adding item into the database as well as to the screen
        axios.post('http://localhost:3000/user/add-expense',expenseDetails)
        .then((res) =>{
            addRowToTable(tableBody, expenseDetails);
        })
        .catch((err) => {
